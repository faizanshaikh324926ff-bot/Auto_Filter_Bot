...
from info import (
    ...
    TMDB_ON_SEARCH, TMDB_POSTER, ULTRA_FAST_MODE, UPDATE_CHNL_LNK, URL, PREMIUM_USER
)
...
DAILY_SEARCH_COLLECTION = mdb["daily_search_limits"]


async def consume_daily_search(user_id: int):
    """Allow 5 free PM searches per user per IST calendar day.
    Admins and configured premium users are unlimited.
    """
    if user_id in ADMINS or user_id in PREMIUM_USER:
        return True, 0
    ...
    try:
        doc = await DAILY_SEARCH_COLLECTION.find_one({"_id": key})

        if not doc or doc.get("date") != today:
            await DAILY_SEARCH_COLLECTION.update_one(
                {"_id": key},
        ...
        result = await DAILY_SEARCH_COLLECTION.update_one(
            {"_id": key, "date": today, "count": {"$lt": DAILY_SEARCH_LIMIT}},
            {"$inc": {"count": 1}},
        )
        ...
        # Another request may have consumed the final search.
        latest = await DAILY_SEARCH_COLLECTION.find_one({"_id": key})
        latest_used = int((latest or {}).get("count", DAILY_SEARCH_LIMIT))
        return False, latest_used

    except Exception:
        logger.exception("Daily search limit error for user %s", user_id)
        # Fail open so a database issue does not break searching.
        return True, 0
...
async def pm_text(bot, message):
    ...
    try:
        ...
        if pm_search:
            allowed, used = await consume_daily_search(user_id)
            if not allowed:
                await message.reply_text(
                    "🚫 <b>ᴅᴀɪʟʏ ꜰʀᴇᴇ sᴇᴀʀᴄʜ ʟɪᴍɪᴛ ʀᴇᴀᴄʜᴇᴅ</b>\n\n"
                    f"Yᴏᴜ ᴄᴀɴ ᴅᴏ <b>{DAILY_SEARCH_LIMIT} sᴇᴀʀᴄʜᴇs ᴘᴇʀ ᴅᴀʏ</b> ғᴏʀ ғʀᴇᴇ.\n"
                    "♻️ ᴛʜᴇ ʟɪᴍɪᴛ ʀᴇsᴇᴛs ᴇᴠᴇʀʏ ᴅᴀʏ ᴀᴛ 12:00 ᴀᴍ (IST).\n\n"
                    "💎 ᴜsᴇ ᴘʀᴇᴍɪᴜᴍ ғᴏʀ ᴜɴʟɪᴍɪᴛᴇᴅ sᴇᴀʀᴄʜᴇs.",
